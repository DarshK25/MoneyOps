...
class BackendHttpAdapter:
    ...
    async def get_clients(self, org_uuid: str, user_id: str = None):
        return await self._request(
            "GET", "/api/clients",
            org_id=org_uuid,
            user_id=user_id or org_uuid,
            params={"limit": 100}
        )
    ...
    async def get_financial_summary(self, org_id: str, user_id: Optional[str] = None) -> BackendResponse:
        """Alias for metrics endpoint to provide high-level balance/profit summary."""
        # Using businessId=1 as default for now as per dashboard fallback
        return await self._request("GET", "/api/finance-intelligence/metrics", params={"businessId": 1}, org_id=org_id, user_id=user_id)
